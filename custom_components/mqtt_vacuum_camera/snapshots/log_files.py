...
async def async_get_filtered_logs(directory_path: str, file_name):
    """Make a copy of home-assistant.log to home-assistant.tmp"""
    log_file_path = os.path.join(directory_path, "home-assistant.log")
...
async def async_get_data(storage_path: str, file_name: str, json_data: Any) -> Any:
    ...
    try:
        ...
        log_data = await async_get_filtered_logs(storage_path, file_name)
        if log_data:
...
async def async_logs_store(hass: HomeAssistant, file_name: str) -> None:
    ...
    vacuum_json = SnapshotStore().async_get_vacuum_json(file_name)
    try:
        # When logger is active.
        if (_LOGGER.getEffectiveLevel() > 0) and (
                _LOGGER.getEffectiveLevel() != 30
        ):
            await async_get_data(storage_path, file_name, vacuum_json)
...
async def run_async_save_logs(hass: HomeAssistant, file_name: str) -> None:
    ...
    num_processes = 1
    pil_img_list = [file_name for _ in range(num_processes)]
    loop = get_event_loop()
