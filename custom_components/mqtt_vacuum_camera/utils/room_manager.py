...
class RoomManager:
    ...
    async def write_vacuum_id(self, vacuum_id: str) -> None:
        ...
        try:
            await asyncio.to_thread(os.makedirs, os.path.dirname(json_path), exist_ok=True)
    ...
    async def rename_room_descriptions(self, vacuum_id: str) -> bool:
        ...
        edit_path = self.hass.config.path("custom_components/mqtt_vacuum_camera/translations")
        _LOGGER.info("Editing the translations file for languages: %s", languages)

        # Load all translation files in one batch
        data_list = await self.language_cache.load_translations_json(self.hass, languages)
        ...
        if None in data_list:
            ...
            if en_translation:
                ...
                data_list = [data if data is not None else en_translation for data in data_list]
    ...
    async def _modify_translation_data(self, data: dict, room_list: List[Tuple[str, dict]]) -> Optional[dict]:
        """
        ...
        for i in range(1, 3):
            ...
            for j in range(start_index, end_index):
                if j < len(room_list):
                    ...
                    modified_data["options"]["step"][room_key]["data_description"][
                        f"color_room_{j}"
        ...
        for i in range(2, 4):
            ...
            for j in range(start_index, end_index):
                if j < len(room_list):
                    room_id, room_info = room_list[j]
                    room_name = room_info.get("name", f"Room {room_id}")
                    modified_data["options"]["step"][alpha_key]["data"][f"alpha_room_{j}"] = (
                        f"RoomID {room_id} {room_name}"
                    )
                else:
                    modified_data["options"]["step"][alpha_key]["data"][f"alpha_room_{j}"] = ""
    ...
    async def _write_json_file(self, file_path: str, data: dict) -> None:
        ...
        try:
            ...
            _LOGGER.warning("Error writing translation file %s: %s", file_path, str(e), exc_info=True)
