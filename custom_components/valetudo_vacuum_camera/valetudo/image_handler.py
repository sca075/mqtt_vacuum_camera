...
class image_handler(Image):
    def __init__(self, json_data, Vacuum):
    ...
    def crop_array(self, image_array, crop_percentage, scale_factor):
    ...
    def get_image_from_json(self, m_json):
        ...
        self.charger_pos = entity_dict.get("charger_location")
        self.charger_pos = self.charger_pos[0]["points"]
        self.go_to = entity_dict.get("go_to_target")
        self.vac_json_id = m_json["metaData"]["nonce"]
        ...
        img_np_array = self.from_json_to_image(self.flour_pixel, self.pixel_size, color_home_background)
        img_np_array = img_np_array + self.from_json_to_image(self.wall_pixel, self.pixel_size, color_wall)
        img_np_array = self.draw_lines(img_np_array, self.path_pixel2, 5, color_move)
        img_np_array = self.draw_battery_charger(img_np_array, self._charger_pos[0], self._selcharger_pos[1], color_charger)
        if self.go_to:
            img_np_array = self.draw_go_to_flag((self.go_to[0]["points"][0], self.go_to[0]["points"][1]), img_np_array)
        img_np_array = img_np_array + self.draw_robot(img_np_array, self.robot_position[0], self.robot_position[1],
                                                 self.robot_position_angle, color_robot)

        img_np_array = self.crop_array(img_np_array, 25, self._image_scale)
